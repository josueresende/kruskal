...
public class Main {
    ...
    static public void main(String... args) {
        ...
        Kruskal.carregar_dados("resource/GrafoCompleto/inst_v100.dat")
            .forEach(instancia -> {
                execute_1(instancia.arestas, instancia.n_nodes);
            });
        Kruskal.carregar_dados("resource/GrafoCompleto/inst_v100.dat")
            .forEach(instancia -> {
                execute_2(instancia.arestas, instancia.n_nodes);
            });
    }
    static private void execute_2(int[][] data, int n_nodes) {
        ...
        Kruskal.quicksort(data, 0 , data.length - 1);
        long s = mx.getCurrentThreadCpuTime();
        Kruskal.execute_union_by_rank(data, parent, rank);
    ...
    static private void execute_1(int[][] data, int n_nodes) {
        ...
        Kruskal.quicksort(data, 0 , data.length - 1);
        long s = mx.getCurrentThreadCpuTime();
        Kruskal.execute_union_find(data, parent);
        long e = mx.getCurrentThreadCpuTime();
        System.out.println(((e - s) * 1e-9) + " seconds");
        // for (int j = 0; j < n_nodes; j++) {
